...
public class SubmitBookForPublishingActivity {
    ...
    public SubmitBookForPublishingResponse execute(SubmitBookForPublishingRequest request) {
        ...
        if(bookId != null) {
//            CatalogDao catalogDao1 = new CatalogDao(new DataAccessModule().provideDynamoDBMapper());
            catalogDao.validateBookExists(bookId);
        } else {
            request.setBookId(UUID.randomUUID().toString());
        }
//      BookPublishRequestManager bookPublishRequestManager1 = new BookPublishRequestManager();
